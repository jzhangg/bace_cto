...
size_thetas  = 2000                      # Size of sample drawn from prior distribuion over preference parameters.
...
theta_params = dict(
    vbar    = scipy.stats.norm(loc=800, scale=100),
    k       = scipy.stats.norm(loc=800, scale=100)
)
...
design_params = dict(
    deposit_a   = scipy.stats.uniform(0, 500),
    deposit_b   = scipy.stats.uniform(0, 500),
    repay_a     = scipy.stats.uniform(0, 100),
    repay_b     = scipy.stats.uniform(0, 100)
)
